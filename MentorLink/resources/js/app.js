...
const hydratePlatformOverview = async () => {
    ...
    try {
        ...
        if (stackCount) {
            stackCount.textContent = data.database ? `${data.database}` : 'Donnees locales';
        }

        const healthLabel = document.querySelector('[data-health-label]');
        if (healthLabel) {
            healthLabel.textContent = data.status === 'ready'
                ? 'Donnees synchronisees avec la plateforme'
                : 'Plateforme prete a afficher vos donnees';
        }
        ...
        if (healthLabel) {
            healthLabel.textContent = 'Connexion non verifiee localement';
        }
